...
function bouncing(target) {
	...
	if (paddleX < target.x + target.width &&
		...
		console.log('speedX:' + target.speedX);
	...
	for (i = 0; i < bricks.length; i++) {
		...
		if (bricks[i].x < target.x + target.width &&
			...
			//experimental without the y limitations. trying to fix the ball crossing the corners. After some testing it seems to be good.
			...
			//not bad but has issues when the ball passes a corner diagonally
			//			if ((target.x) <= (bricks[i].x + bricks[i].width) && target.y > bricks[i].y && target.y < bricks[i].y + bricks[i].height && bricks[i].x + bricks[i].width / 2 < target.x && target.x >= bricks[i].x + bricks[i].width - 5) {
			//				// -5 this is how further in the brick the ball can go its not 4 because when you hit with the side of the paddle values go over 100 and speed goes a bit above 4. each frame they move with 4px so they might align with the border or go up to ~4px in the brick.
			//				console.log('right');
			//				target.speedX = Math.abs(target.speedX);
			//			}
			//			if ((target.x + target.width >= bricks[i].x) && target.y > bricks[i].y && target.y < bricks[i].y + bricks[i].height && bricks[i].x + bricks[i].width / 2 > target.x && target.x + target.width <= bricks[i].x + 5) {
			//				target.speedX = -Math.abs(target.speedX);
			//				console.log('left');
			//			}
			//			if ((target.y + target.height) >= bricks[i].y && target.y < bricks[i].y && target.y + target.height <= bricks[i].y + 5) {
